...
if __name__=="__main__":
    ...
    while not rospy.is_shutdown():
        ...
        if ready2navigate:
            ...
            dx,dy,_ = np.array(BB.position)-np.array(Kfetch.position)
            goal_yaw = np.arctan(dy/dx)*(180.0/np.pi)
        ...
        if ready2deliver:
            rospy.loginfo("\n\n####### begining delivery ######")
            rospy.loginfo([ready2pick,ready2deliver,ready2place])
